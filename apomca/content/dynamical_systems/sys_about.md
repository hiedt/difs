...
- (1) means that, the next state in time depends on the current state.
- (2) means that, the initial condition of the system (state at t = 0) must be a known and unique constant.
...
## Characters of a dynamical system

The very first job in every control problem is to analyze a system's characters such as linearity, controllability, and so on.
